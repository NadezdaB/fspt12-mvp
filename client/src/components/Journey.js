...
export default function Journey() {
  ...
  const showStatistics = async () => {
    ...
    var layout = {
      xaxis: {
        range: [0, Math.max(...xArray) * 1.2],
        title: 'Covered distance, km',
      },
  ...
  const showStationStats = async () => {
    ...
    let filterDeparture = journeys.filter(
      (e) => e.departureStationName === journey[0].departureStationName
    )
    let filterReturn = journeys.filter(
      (e) => e.returnStationName === journey[0].returnStationName
    )
    let depHours = filterDeparture.map(
      (journey) =>
        parseInt(DateTime.fromISO(journey.departureTime).toFormat('HH')) - 3
    )
    let retHours = filterDeparture.map(
      (journey) =>
        parseInt(DateTime.fromISO(journey.returnTime).toFormat('HH')) - 3
    )

    let depHoursRetStation = filterReturn.map(
      (journey) =>
        parseInt(DateTime.fromISO(journey.departureTime).toFormat('HH')) - 3
    )
    let retHoursRetStation = filterReturn.map(
      (journey) =>
        parseInt(DateTime.fromISO(journey.returnTime).toFormat('HH')) - 3
    )
    ...
    let nameForPlot = [
      `Departure hours for departure station ${journey[0].departureStationName}`,
  ...
  const deleteJourney = async () => {
    ...
    const row = document.getElementsByClassName('highlight')
    console.log(
      'This is id of the row to be deleted',
      row[0].getAttribute('id')
    )
    let rowID = row[0].getAttribute('id')
  ...
  return (
    ...
    >
      ...
      <button onClick={showStationStats}>
        Show statistics on the selected stations
      </button>

      <h2 id='plotDistance' className='plot'>
        Statistics on covered distance and duration
      </h2>
      <br />
      <h2 id='myDiv' className='plot'>
        Statistics on selected departure and return stations
      </h2>
    </div>
  )
}
